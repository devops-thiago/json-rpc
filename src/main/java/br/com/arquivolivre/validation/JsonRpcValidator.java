...
/**
 * Centralized validation logic for JSON-RPC objects.
 * Provides static utility methods for validating JSON-RPC 2.0 protocol constraints.
 */
public final class JsonRpcValidator {
    ...
    public static void validateMethod(String method) {
        ...
        if (method.startsWith("rpc.")) {
            throw new JsonRpcException(
                JsonRpcError.of(-32600, "Invalid Request: method names starting with 'rpc.' are reserved")
            );
        }
    }
    ...
    public static void validateId(Object id) {
        ...
        if (id instanceof Number numId) {
            ...
            if (doubleValue != Math.floor(doubleValue)) {
                System.err.println("Warning: JSON-RPC id contains fractional parts, which is discouraged: " + id);
            }
