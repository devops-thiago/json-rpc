...
public class JsonRpcResponseAdapter extends TypeAdapter<JsonRpcResponse<?>> {
    ...
    public JsonRpcResponse<?> read(JsonReader in) throws IOException {
        ...
        if (!"2.0".equals(jsonrpc)) {
            throw new JsonRpcException(StandardErrors.invalidRequest("Invalid 'jsonrpc' version: " + jsonrpc));
        }
        ...
        if (hasResult && hasError) {
            throw new JsonRpcException(StandardErrors.invalidRequest("Response cannot have both 'result' and 'error'"));
        }
        if (!hasResult && !hasError) {
            throw new JsonRpcException(StandardErrors.invalidRequest("Response must have either 'result' or 'error'"));
        }
