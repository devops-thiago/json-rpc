...
public class JsonRpcErrorAdapter extends TypeAdapter<JsonRpcError<?>> {
    ...
    public JsonRpcError<?> read(JsonReader in) throws IOException {
        ...
        if (!jsonObject.has("code")) {
            throw new JsonRpcException(StandardErrors.invalidRequest("Missing 'code' field in error object"));
        }
        if (!jsonObject.get("code").isJsonPrimitive() || !jsonObject.get("code").getAsJsonPrimitive().isNumber()) {
            throw new JsonRpcException(StandardErrors.invalidRequest("Invalid 'code' field: must be a number"));
        }
        var code = jsonObject.get("code").getAsInt();
        
        // Validate message field
        if (!jsonObject.has("message")) {
            throw new JsonRpcException(StandardErrors.invalidRequest("Missing 'message' field in error object"));
        }
        if (!jsonObject.get("message").isJsonPrimitive() || !jsonObject.get("message").getAsJsonPrimitive().isString()) {
            throw new JsonRpcException(StandardErrors.invalidRequest("Invalid 'message' field: must be a string"));
        }
